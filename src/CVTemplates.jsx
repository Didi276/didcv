...
// ─── Calibration automatique : interligne + marges distribuent l'espace ─────
// CV léger → lineH grand + mb grand = contenu aéré qui remplit la page
// CV dense → lineH serré + mb petit = tout tient sans déborder (overflow:hidden)
function getFontConfig(cvData) {
  ...
  const density = nbExp
    ...
    + (hasCert ? 1.5 : 0) + (hasCI ? 1 : 0)
    + nbFormations * 0.5 + (hasFormDesc ? 1 : 0)
  ...
  if (density < 5)  return { base: '11px', small: '10px', xsmall: '9px', lineH: '3.3', mb: '28px' }
  if (density < 7)  return { base: '11px', small: '10px', xsmall: '9px', lineH: '2.92', mb: '23px' }
  if (density < 9)  return { base: '11px', small: '10px', xsmall: '9px', lineH: '2.5', mb: '19px' }
  if (density < 11) return { base: '11px', small: '10px', xsmall: '9px', lineH: '2.18', mb: '15px' }
  if (density < 14) return { base: '11px', small: '10px', xsmall: '8.5px', lineH: '2.0', mb: '13px' }
  if (density < 17) return { base: '10.5px', small: '9.5px', xsmall: '8px', lineH: '1.73', mb: '9px' }
  if (density < 21) return { base: '10px', small: '9px', xsmall: '7.5px', lineH: '1.5', mb: '6px' }
    ...
    <div id="cv-to-print" style={{fontFamily:'Georgia,serif',color:'#1a1a1a',fontSize:f.base,lineHeight:f.lineH,padding:'36px 40px',background:'#fff',width:'794px',height:'1123px',overflow:'hidden',boxSizing:'border-box'}}>
        ...
        {cvData.experiences?.map((exp,i)=>(
          <div key={i} style={{marginBottom:f.mb}}>
            ...
            <ul style={{paddingLeft:'16px',marginTop:'4px',marginBottom:0}}>
              {exp.missions?.map((m,j)=><li key={j} style={{fontSize:f.small,color:'#333',marginBottom:'3px',lineHeight:f.lineH}}>{m}</li>)}
            </ul>
          </div>
        ))}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px',marginBottom:'10px'}}>
        ...
        <div>
          ...
          {cvData.langues?.map((l,i)=><div key={i} style={{display:'flex',justifyContent:'space-between',fontSize:f.small,borderBottom:'1px solid #eee',padding:'3px 0'}}><span>{l.langue}</span><span style={{color:'#777',fontStyle:'italic'}}>{l.niveau}</span></div>)}
        </div>
      </div>
        ...
        <div style={{marginBottom:f.mb}}>
          ...
          {cvData.experiences?.map((exp,i)=>(
            <div key={i} style={{marginBottom:f.mb,paddingLeft:'10px',borderLeft:'2px solid #e0e0e0'}}>
              ...
              <ul style={{paddingLeft:'12px',marginTop:'4px',marginBottom:0}}>{exp.missions?.map((m,j)=><li key={j} style={{fontSize:f.small,color:'#444',marginBottom:'2px',lineHeight:f.lineH}}>{m}</li>)}</ul>
            </div>
          ))}
        </div>
        <div style={{marginBottom:f.mb}}>
          ...
          {cvData.formations?.map((f2,i)=>(
            <div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:f.mb,paddingLeft:'10px',borderLeft:'2px solid #e0e0e0'}}>
    ...
    <div id="cv-to-print" style={{fontFamily:'"Helvetica Neue",Helvetica,sans-serif',fontSize:f.base,lineHeight:f.lineH,background:'#fff',width:'794px',height:'1123px',overflow:'hidden',display:'grid',gridTemplateColumns:'250px 1fr'}}>
      <div style={{background:'#2d2d2d',color:'#fff',padding:'28px 20px'}}>
        ...
        <div style={{marginBottom:'12px'}}>
          ...
          {cvData.langues?.map((l,i)=><div key={i} style={{fontSize:f.xsmall,color:'rgba(255,255,255,0.85)',marginBottom:'4px'}}><strong>{l.langue}</strong> — {l.niveau}</div>)}
        </div>
        ...
        <div style={{marginBottom:f.mb}}>
          ...
          {cvData.experiences?.map((exp,i)=>(
            <div key={i} style={{marginBottom:f.mb,paddingLeft:'10px',borderLeft:'3px solid #f093fb'}}>
              ...
              <ul style={{paddingLeft:'10px',marginTop:'3px',marginBottom:0}}>{exp.missions?.map((m,j)=><li key={j} style={{fontSize:f.small,color:'#555',marginBottom:'2px',lineHeight:f.lineH}}>{m}</li>)}</ul>
            </div>
          ))}
        </div>
        <div>
          ...
          {cvData.formations?.map((f2,i)=><div key={i} style={{marginBottom:f.mb,paddingLeft:'10px',borderLeft:'3px solid #f093fb'}}><div style={{fontWeight:'700',fontSize:f.base}}>{f2.diplome}</div><div style={{fontSize:f.small,color:'#888'}}>{f2.etablissement} · {f2.periode}</div></div>)}
        </div>
      </div>
    </div>
  )
      ...
      <div style={{marginBottom:f.mb}}>
        ...
        {cvData.experiences?.map((exp,i)=>(
          <div key={i} style={{marginBottom:f.mb}}>
            ...
            <ul style={{paddingLeft:'16px',margin:0}}>{exp.missions?.map((m,j)=><li key={j} style={{fontSize:f.small,color:'#222',marginBottom:'2px',lineHeight:f.lineH}}>{m}</li>)}</ul>
          </div>
        ))}
      </div>
      <div style={{marginBottom:f.mb}}>
        ...
        {cvData.formations?.map((f2,i)=><div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:f.mb}}><div><div style={{fontWeight:'700',fontSize:f.base}}>{f2.etablissement}</div><div style={{fontSize:f.small,fontStyle:'italic',color:'#333'}}>{f2.diplome}</div></div><div style={{fontSize:f.xsmall,color:'#555'}}>{f2.periode}</div></div>)}
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'14px',marginBottom:'10px'}}>
        <div><div style={{fontSize:f.small,fontWeight:'700',textTransform:'uppercase',letterSpacing:'1px',borderBottom:'1px solid #111',paddingBottom:'2px',marginBottom:'7px'}}>Skills</div><div style={{fontSize:f.small,color:'#222',lineHeight:f.lineH}}>{cvData.competences?.join(' · ')}</div></div>
        <div><div style={{fontSize:f.small,fontWeight:'700',textTransform:'uppercase',letterSpacing:'1px',borderBottom:'1px solid #111',paddingBottom:'2px',marginBottom:'7px'}}>Languages</div>{cvData.langues?.map((l,i)=><div key={i} style={{fontSize:f.small,color:'#222',marginBottom:'3px'}}>{l.langue} — {l.niveau}</div>)}</div>
      </div>
  ...
  return (
    <div id="cv-to-print" style={{fontFamily:'-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif',fontSize:f.base,lineHeight:f.lineH,background:'#fff',color:'#1d1d1f',width:'794px',height:'1123px',overflow:'hidden',padding:'36px 44px',boxSizing:'border-box'}}>
      ...
      <div style={{marginBottom:f.mb}}>
        ...
        {cvData.experiences?.map((exp,i)=>(
          <div key={i} style={{display:'grid',gridTemplateColumns:'100px 1fr',gap:'14px',marginBottom:f.mb}}>
            <div style={{fontSize:f.xsmall,color:'#6e6e73',paddingTop:'1px'}}>{exp.periode}</div>
            <div><div style={{fontWeight:'600',fontSize:f.base}}>{exp.poste}</div><div style={{fontSize:f.small,color:'#6e6e73',marginBottom:'4px'}}>{exp.entreprise} · {exp.lieu}</div><ul style={{paddingLeft:'12px',margin:0}}>{exp.missions?.map((m,j)=><li key={j} style={{fontSize:f.small,color:'#3d3d3f',marginBottom:'2px',lineHeight:f.lineH}}>{m}</li>)}</ul></div>
          </div>
        ))}
      </div>
      <div style={{marginBottom:f.mb}}>
        ...
        {cvData.formations?.map((f2,i)=><div key={i} style={{display:'grid',gridTemplateColumns:'100px 1fr',gap:'14px',marginBottom:f.mb}}><div style={{fontSize:f.xsmall,color:'#6e6e73'}}>{f2.periode}</div><div><div style={{fontWeight:'600',fontSize:f.base}}>{f2.diplome}</div><div style={{fontSize:f.small,color:'#6e6e73'}}>{f2.etablissement}</div></div></div>)}
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'20px',marginBottom:'10px'}}>
        ...
        <div><div style={{fontSize:f.xsmall,fontWeight:'600',letterSpacing:'2px',textTransform:'uppercase',color:'#6e6e73',marginBottom:'8px'}}>LANGUES</div>{cvData.langues?.map((l,i)=><div key={i} style={{fontSize:f.small,color:'#3d3d3f',marginBottom:'4px'}}>{l.langue} <span style={{color:'#6e6e73'}}>— {l.niveau}</span></div>)}</div>
      </div>
...
export function TemplateCorporate({ cvData }) {
  ...
  return (
    <div id="cv-to-print" style={{fontFamily:'Arial,Helvetica,sans-serif',fontSize:f.base,lineHeight:f.lineH,background:'#fff',width:'794px',height:'1123px',overflow:'hidden'}}>
      ...
      {cvData.accroche && <div style={{marginBottom:'0',padding:'12px 32px',background:'#f0f4f8',borderBottom:'1px solid #e2e8f0'}}><p style={{fontSize:f.small,color:'#334155',margin:0,lineHeight:f.lineH}}>{cvData.accroche}</p></div>}
