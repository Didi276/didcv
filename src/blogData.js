export const ARTICLES = [
  {
    slug: 'comment-faire-un-bon-cv',
    titre: 'Comment faire un bon CV en 2025 : le guide complet',
    description: 'Tous les conseils pour créer un CV professionnel qui attire les recruteurs et passe les filtres ATS.',
    date: '2025-06-01',
    categorie: 'CV',
    duree: '8 min',
    contenu: `
## Pourquoi votre CV est crucial

Les recruteurs passent en moyenne **6 secondes** sur un CV avant de décider de le lire ou de le rejeter. Votre document doit donc être impactant dès le premier regard.

## Les 7 règles d'or d'un bon CV

### 1. Une page maximum (si moins de 10 ans d'expérience)
Un CV d'une page est la norme en France pour les profils juniors et intermédiaires. Allez à l'essentiel.

### 2. Un titre professionnel clair
Placez votre titre exactement sous votre nom. Il doit correspondre au poste visé. Ex : "Développeur React Senior" plutôt que "Passionné de technologie".

### 3. Une accroche percutante en 3-4 phrases
L'accroche résume votre profil, votre expérience clé et votre valeur ajoutée. C'est votre argument de vente principal.

### 4. Des missions quantifiées
Évitez les missions vagues. Préférez : "Augmentation du CA de 23% en 6 mois" plutôt que "Développement commercial".

### 5. Les compétences adaptées au poste
Lisez attentivement l'offre d'emploi et intégrez les mots-clés dans votre CV. Les ATS scannent ces termes.

### 6. Un design lisible et épuré
Police claire (Inter, Calibri, Arial), taille 10-12pt, espacement aéré, sans fioritures inutiles.

### 7. Format PDF texte réel
Toujours exporter en PDF avec du texte sélectionnable, jamais en image. Les ATS ne peuvent pas lire les PDFs image.

## Les sections indispensables

- **Informations personnelles** : nom, email, téléphone, ville, LinkedIn
- **Titre professionnel**
- **Accroche / profil**
- **Expériences professionnelles** (ordre antéchronologique)
- **Formations**
- **Compétences**
- **Langues**

## Ce qu'il faut éviter

- Photo (sauf si le secteur le demande)
- Informations personnelles inutiles (état civil, âge)
- Fautes d'orthographe
- Mensonges ou approximations
- Templates trop créatifs qui trompent les ATS
    `
  },
  {
    slug: 'optimiser-cv-ats',
    titre: 'Comment optimiser son CV pour les ATS en 2025',
    description: 'Guide complet pour passer les filtres ATS et augmenter vos chances d\'être sélectionné.',
    date: '2025-06-05',
    categorie: 'ATS',
    duree: '6 min',
    contenu: `
## Qu'est-ce qu'un ATS ?

Un ATS (Applicant Tracking System) est un logiciel utilisé par **75% des entreprises** pour filtrer automatiquement les CVs avant qu'un recruteur humain ne les lise.

## Comment fonctionne un ATS ?

L'ATS analyse votre CV à la recherche de mots-clés correspondant à l'offre d'emploi. Il attribue un score et seuls les CV au-dessus d'un certain seuil sont transmis aux recruteurs.

## Les règles pour passer les ATS

### Utilisez les bons mots-clés
Lisez l'offre d'emploi mot par mot et identifiez les compétences requises. Intégrez-les naturellement dans votre CV.

### Structure simple et standard
- Titres de sections classiques : "Expériences", "Formations", "Compétences"
- Évitez les tableaux, les colonnes multiples complexes
- Pas d'en-têtes ou pieds de page (les ATS ne les lisent pas)

### Format PDF texte
Votre CV doit être un vrai PDF avec du texte sélectionnable, jamais une image.

### Police standard
Utilisez Arial, Calibri, Times New Roman ou Inter. Évitez les polices décoratives.

## Score ATS : comment l'améliorer ?

DidCV propose une analyse ATS intégrée qui évalue votre CV contre l'offre d'emploi et vous donne un score détaillé avec les mots-clés manquants.
    `
  },
  {
    slug: 'lettre-motivation-exemple',
    titre: 'Lettre de motivation : exemple et conseils pour 2025',
    description: 'Comment rédiger une lettre de motivation efficace avec des exemples concrets par secteur.',
    date: '2025-06-10',
    categorie: 'Lettre',
    duree: '7 min',
    contenu: `
## La lettre de motivation est-elle encore utile ?

Oui. Malgré ce qu'on entend souvent, **60% des recruteurs** lisent encore la lettre de motivation. Elle fait la différence entre deux profils similaires.

## La structure idéale

### Paragraphe 1 : L'accroche
Montrez que vous connaissez l'entreprise et expliquez pourquoi ce poste vous intéresse spécifiquement.

### Paragraphe 2 : Votre valeur ajoutée
Illustrez votre expérience la plus pertinente avec un exemple concret et quantifié.

### Paragraphe 3 : La motivation pour l'entreprise
Montrez que vous avez fait des recherches sur l'entreprise, ses valeurs, ses produits.

### Conclusion
Proposez un entretien et remerciez pour l'attention portée à votre candidature.

## Les erreurs à éviter

- Commencer par "Je me permets de vous contacter", une formule trop banale
- Reformuler le CV dans la lettre
- Lettre générique non personnalisée
- Dépasser une page
- Fautes d'orthographe

## Exemple de bonne accroche

"Après 5 ans à développer des stratégies marketing B2B chez [Entreprise X], c'est avec enthousiasme que je postule au poste de Responsable Marketing chez [Entreprise Y]. Votre approche innovante de [sujet] m'a particulièrement marqué lors de [événement/article]."
    `
  },
  {
    slug: 'cv-sans-experience-etudiant',
    titre: 'CV étudiant sans expérience : comment le valoriser en 2025',
    description: 'Conseils et astuces pour créer un CV percutant quand on manque d\'expérience professionnelle.',
    date: '2025-06-15',
    categorie: 'Étudiant',
    duree: '5 min',
    contenu: `
## Le CV étudiant sans expérience : ce n'est pas une fatalité

Tous les professionnels ont commencé sans expérience. La clé est de valoriser ce que vous avez : formations, projets, compétences, engagements.

## Ce que vous pouvez mettre à la place des expériences

### Projets académiques
Les projets de fin d'études, travaux pratiques et projets personnels comptent. Décrivez-les comme des expériences : objectif, méthodes, résultats.

### Stages et jobs étudiants
Même un stage de 2 mois est une expérience. Même un job d'été au supermarché développe des compétences : rigueur, travail en équipe, gestion du temps.

### Associations et bénévolat
Président d'une association étudiante, organisation d'événements, bénévolat : tout cela parle de votre personnalité et de vos soft skills.

### Compétences techniques
Maîtrise de logiciels, langages de programmation, outils bureautiques. Mettez tout en valeur.

## Structure recommandée pour un CV étudiant

1. Formation (en premier si récente)
2. Compétences
3. Projets / Expériences
4. Langues
5. Centres d'intérêt (sélectifs et pertinents)

## Conseil DidCV

Notre template "Étudiant" est spécialement conçu pour valoriser les formations et projets plutôt que les expériences.
    `
  },
  {
    slug: 'cv-alternance-guide',
    titre: 'CV pour l\'alternance : le guide complet 2025',
    description: 'Comment créer le CV parfait pour une candidature en alternance ou apprentissage.',
    date: '2025-06-20',
    categorie: 'Alternance',
    duree: '6 min',
    contenu: `
## L'alternance : un format qui séduit les recruteurs

L'alternance combine formation et expérience professionnelle. Les recruteurs savent que vous avez des bases théoriques solides ET une première immersion en entreprise.

## Les spécificités du CV alternance

### Mentionnez clairement le rythme
Indiquez votre rythme d'alternance : "3 semaines entreprise / 1 semaine école" ou "50/50". Les recruteurs ont besoin de planifier.

### Précisez la durée
"Contrat d'apprentissage de 24 mois à partir de septembre 2025"

### Votre école et votre formation
En alternance, votre formation est aussi importante que votre expérience. Mentionnez le nom complet de votre école, le titre du diplôme préparé et la promotion.

## Ce que recherchent les recruteurs en alternance

- Motivation et sérieux
- Capacité d'apprentissage rapide
- Compétences de base maîtrisées
- Compatibilité avec l'équipe

## Template recommandé

DidCV propose un template "Alternance" qui met en avant à la fois votre formation et vos expériences de manière équilibrée.
    `
  },
  {
    slug: 'reussir-entretien-embauche',
    titre: 'Comment réussir un entretien d\'embauche : 15 conseils',
    description: 'Tout ce qu\'il faut savoir avant, pendant et après un entretien d\'embauche pour décrocher le poste.',
    date: '2025-06-25',
    categorie: 'Entretien',
    duree: '9 min',
    contenu: `
## Avant l'entretien

### 1. Recherchez l'entreprise en profondeur
Connaître les produits, l'actualité, les valeurs et les concurrents de l'entreprise vous permet de poser des questions pertinentes et de montrer votre motivation.

### 2. Relisez votre CV et la fiche de poste
Vous devez pouvoir développer chaque point de votre CV et connecter vos expériences aux besoins du poste.

### 3. Préparez la question "Parlez-moi de vous"
C'est souvent la première question. Préparez un pitch de 2-3 minutes : qui vous êtes, votre parcours, pourquoi ce poste.

### 4. Préparez des exemples concrets
La méthode STAR : Situation, Tâche, Action, Résultat. Pour chaque compétence clé, ayez un exemple précis.

## Pendant l'entretien

### 5. Soignez votre apparence
Adaptez votre tenue au secteur. En cas de doute, soyez légèrement plus habillé que nécessaire.

### 6. Arrivez 10 minutes avant
Ni trop tôt (embarrassant pour le recruteur) ni en retard.

### 7. Langage corporel positif
Poignée de main ferme, contact visuel, posture droite, sourire naturel.

### 8. Écoutez vraiment les questions
Prenez 2-3 secondes avant de répondre. Il vaut mieux une réponse courte et pertinente qu'une longue réponse à côté.

### 9. Posez des questions
À la fin, posez 2-3 questions sur l'équipe, les projets, les prochaines étapes. Montrez votre intérêt.

## Après l'entretien

### 10. Envoyez un email de remerciement
Dans les 24h, envoyez un court email pour remercier et réaffirmer votre motivation.

### 11. Notez ce qui s'est passé
Pour les prochains entretiens et pour votre suivi de candidatures.
    `
  },
  {
    slug: 'cv-reconversion-professionnelle',
    titre: 'CV pour une reconversion professionnelle : stratégies et conseils',
    description: 'Comment présenter votre CV lors d\'une reconversion pour valoriser votre expérience passée.',
    date: '2025-07-01',
    categorie: 'Reconversion',
    duree: '7 min',
    contenu: `
## La reconversion : un atout, pas un handicap

Les recruteurs voient de plus en plus positivement les profils en reconversion. Ils apportent une expérience diversifiée, une maturité et souvent une motivation exceptionnelle.

## Stratégie pour le CV de reconversion

### Mettez les compétences transférables en avant
Identifiez les compétences de votre ancien métier qui sont utiles dans votre nouveau domaine. Gestion de projet, management, relation client, analyse de données : ces compétences traversent les secteurs.

### Structurez autour des compétences, pas du temps
Considérez un CV "fonctionnel" qui met les compétences en avant plutôt qu'une liste chronologique d'expériences.

### Expliquez votre reconversion dans l'accroche
Soyez transparent et positif. "Fort d'une expérience de 10 ans dans la finance, j'ai choisi de me reconvertir dans le développement web pour allier ma rigueur analytique à ma passion pour la technologie."

### Montrez votre investissement dans la formation
Formations, certifications, projets personnels, bootcamps : tout ce qui montre que vous avez activement préparé votre reconversion.

## Ce que les recruteurs attendent

- Une motivation clairement expliquée
- Une logique dans votre parcours
- Des compétences transférables identifiées
- Une preuve de votre engagement (formation, projets)
    `
  },
  {
    slug: 'mots-cles-cv',
    titre: 'Les mots-clés à mettre dans son CV selon son secteur',
    description: 'Liste des mots-clés essentiels par secteur pour optimiser votre CV et passer les filtres ATS.',
    date: '2025-07-05',
    categorie: 'ATS',
    duree: '5 min',
    contenu: `
## Pourquoi les mots-clés sont essentiels

Les ATS filtrent les CVs en cherchant des mots-clés spécifiques. Sans les bons termes, votre CV peut être rejeté automatiquement même si vous êtes le candidat idéal.

## Mots-clés par secteur

### Finance & Comptabilité
Bilan, compte de résultat, trésorerie, audit, conformité, reporting, Excel, SAP, ERP, consolidation, IFRS, liasse fiscale, TVA, budget, forecast, contrôle de gestion.

### Tech & Informatique
Python, JavaScript, React, Node.js, SQL, API, DevOps, CI/CD, Agile, Scrum, cloud, AWS, Docker, machine learning, data science.

### Marketing & Communication
SEO, SEA, Google Analytics, content marketing, réseaux sociaux, CRM, Hubspot, Salesforce, A/B testing, conversion, ROI, KPI.

### Commerce & Vente
Prospection, closing, CRM, pipeline, chiffre d'affaires, objectifs, fidélisation, B2B, B2C, négociation, portefeuille clients.

### Ressources Humaines
Recrutement, GPEC, paie, formation, droit social, ADP, Workday, onboarding, marque employeur, entretien annuel.

### Santé
Soins infirmiers, pharmacologie, dossier patient, CPAM, protocoles, hygiène hospitalière, bloc opératoire, urgences.

## Comment intégrer les mots-clés naturellement

Ne faites pas une liste de mots-clés. Intégrez-les dans vos missions et compétences de façon naturelle.
    `
  },
  {
    slug: 'cv-cadre-experience',
    titre: 'CV pour cadre expérimenté : comment se démarquer après 10 ans',
    description: 'Stratégies pour mettre en valeur une longue carrière sans surcharger votre CV.',
    date: '2025-07-10',
    categorie: 'Cadre',
    duree: '6 min',
    contenu: `
## Le challenge du profil senior

Avec 10-20 ans d'expérience, vous avez beaucoup à raconter. Le défi : tout condenser en 2 pages maximum sans perdre les informations cruciales.

## Règles pour le CV cadre

### Maximum 2 pages
Au-delà de 10 ans d'expérience, 2 pages sont acceptables. Mais pas 3 ou 4.

### Ne remontez pas avant 15 ans
Les expériences de plus de 15-20 ans peuvent être condensées en une seule ligne ou supprimées si non pertinentes.

### Quantifiez vos résultats
À votre niveau, les chiffres sont indispensables. Équipes managées, budgets gérés, croissance générée.

### Mettez en avant votre vision stratégique
Au-delà des tâches opérationnelles, montrez votre capacité à prendre des décisions stratégiques et à créer de la valeur.

### Soignez particulièrement l'accroche
Votre accroche doit résumer en 4 phrases votre expertise unique et la valeur que vous apportez.

## Ce que les recruteurs cherchent chez un profil senior

- Leadership et management d'équipes
- Vision stratégique et résultats business
- Adaptabilité et gestion du changement
- Réseaux et partenariats développés
    `
  },
  {
    slug: 'cv-international-anglais',
    titre: 'CV international et CV en anglais : les différences clés',
    description: 'Comment adapter votre CV pour postuler à l\'international ou dans une entreprise anglophone.',
    date: '2025-07-12',
    categorie: 'International',
    duree: '5 min',
    contenu: `
## CV français vs CV anglais : les différences fondamentales

### Format et longueur
Le CV américain (résumé) est strictement 1 page. Le CV britannique (CV) peut faire 2 pages. Le CV français peut inclure une photo, pas le CV américain.

### La photo
En France, la photo est courante mais non obligatoire. Aux États-Unis, elle est déconseillée voire interdite (discrimination). Au Royaume-Uni, même chose.

### Les informations personnelles
Ne mettez jamais votre âge, état civil ou nationalité sur un CV américain ou britannique. En France, c'est moins strict mais évitez quand même.

### Le ton
Le CV anglais est plus direct et orienté résultats. Commencez vos missions par des verbes d'action (Managed, Developed, Led, Increased).

## Vocabulaire clé pour CV en anglais

- Expériences : Work Experience / Professional Experience
- Formations : Education
- Compétences : Skills
- Langues : Languages
- Références : References available upon request

## Template DidCV International

Notre template "International" est conçu pour s'adapter aux codes du recrutement international, avec une mise en page sobre et professionnelle adaptée aux marchés anglophones.
    `
  },
  {
    slug: 'salaire-negociation',
    titre: 'Comment négocier son salaire : guide pratique 2025',
    description: 'Stratégies et scripts pour négocier votre salaire en entretien d\'embauche et obtenir ce que vous méritez.',
    date: '2025-07-13',
    categorie: 'Entretien',
    duree: '7 min',
    contenu: `
## La négociation salariale : une étape incontournable

**76% des recruteurs** ont une marge de négociation. Ne pas négocier, c'est laisser de l'argent sur la table.

## Quand parler du salaire ?

Idéalement, laissez le recruteur aborder le sujet en premier. Si la question arrive tôt dans le processus, vous pouvez demander : "Pourriez-vous me préciser la fourchette prévue pour ce poste ?"

## Comment se préparer

### Recherchez les salaires du marché
Consultez les données de l'APEC, Glassdoor, LinkedIn Salary ou Welcome to the Jungle pour votre poste, secteur et zone géographique.

### Définissez vos 3 chiffres
- Votre minimum absolu (en dessous duquel vous refusez)
- Votre cible (ce que vous voulez vraiment)
- Votre fourchette haute (pour avoir de la marge)

## Scripts de négociation

**Si on vous demande vos prétentions :**
"Sur la base de mon expérience de X ans dans [domaine] et du marché actuel, je vise une rémunération entre X et Y€ annuels. Est-ce compatible avec votre budget ?"

**Si l'offre est en dessous de vos attentes :**
"Je suis très intéressé par ce poste et l'entreprise. L'offre est légèrement en dessous de mes attentes. Seriez-vous en mesure d'envisager X€ ?"

## Les éléments à négocier au-delà du salaire

Télétravail, jours de RTT, prime annuelle, tickets restaurant, mutuelle, formation, voiture de fonction.
    `
  },
  {
    slug: 'erreurs-cv-a-eviter',
    titre: '15 erreurs qui font rejeter votre CV immédiatement',
    description: 'Les erreurs les plus courantes dans les CVs français et comment les éviter pour maximiser vos chances.',
    date: '2025-07-14',
    categorie: 'CV',
    duree: '6 min',
    contenu: `
## Les erreurs qui coûtent des entretiens

### 1. Les fautes d'orthographe
Rédhibitoire. Utilisez un correcteur orthographique ET faites relire par quelqu'un.

### 2. Un email non professionnel
"titeuf2003@hotmail.fr" est éliminatoire. Créez une adresse prénom.nom@gmail.com.

### 3. Une photo inappropriée
Selfie, photo de vacances, photo floue. Utilisez une photo professionnelle ou n'en mettez pas.

### 4. Un CV image (non lisible par les ATS)
Généré depuis Canva ou créé depuis une image. Les ATS ne lisent pas les images.

### 5. Des missions trop vagues
"Gestion de l'équipe" ne dit rien. Combien de personnes ? Quels résultats ?

### 6. Un CV surchargé
Trop d'informations = le recruteur ne lit rien. Allez à l'essentiel.

### 7. La même candidature pour tous les postes
Adaptez au minimum le titre et les compétences pour chaque offre.

### 8. Mentir sur ses compétences
Vous serez testé en entretien. Un mensonge découvert = fin de la candidature.

### 9. Des trous non expliqués
Si vous avez eu une période sans emploi, expliquez-la (formation, projet personnel, bénévolat).

### 10. Un format incompatible
Toujours envoyer en PDF, jamais en Word (sauf demande explicite).

### 11. Pas de chiffres
Sans résultats quantifiés, vos missions n'ont pas d'impact.

### 12. Un CV trop long
1 page pour moins de 10 ans d'expérience, 2 pages maximum sinon.

### 13. Des sections inutiles
"Objectifs professionnels" ou "Références disponibles sur demande", supprimez-les.

### 14. Police illisible ou trop petite
Minimum 10pt. Police standard (Inter, Arial, Calibri).

### 15. Pas de personnalisation pour l'ATS
Sans les mots-clés de l'offre, votre CV sera filtré avant d'être lu.
    `
  },
  {
    slug: 'cv-commercial-exemple',
    titre: 'CV commercial : exemple et conseils 2025',
    description: 'Comment structurer un CV commercial orienté résultats avec des exemples concrets pour décrocher un entretien.',
    date: '2025-07-16',
    categorie: 'CV',
    duree: '6 min',
    contenu: `
## Ce qui différencie un bon CV commercial

Un recruteur qui lit un CV commercial cherche une seule chose avant tout le reste : **des résultats chiffrés**. Le discours doit être aussi convaincant que celui que vous tiendriez face à un client.

## La structure qui fonctionne

### Une accroche orientée résultats
"Commercial B2B avec 6 ans d'expérience, spécialisé dans le développement de portefeuille grands comptes. 115% des objectifs atteints en moyenne sur les 3 dernières années."

### Des missions quantifiées
Chaque ligne d'expérience doit répondre à la question "avec quel résultat ?" :
- "Développement d'un portefeuille de 120 clients professionnels"
- "Atteinte de 115% des objectifs de vente sur 3 années consécutives"
- "Négociation et signature de contrats jusqu'à 500K€"

### Les compétences à mettre en avant
Prospection B2B/B2C, closing, négociation, gestion de portefeuille, CRM (Salesforce, HubSpot), présentation produit.

## Exemple de CV commercial réussi

**Accroche :** "Commercial terrain avec 5 ans d'expérience dans la vente de solutions SaaS. Expert en cycle de vente complexe et négociation grands comptes."

**Expérience :** "Responsable Commercial chez TechSales (2021-2024) : Développement d'un portefeuille de 80 clients PME, croissance du CA de 32% en 2 ans, management d'une équipe de 3 SDR."

## Les erreurs à éviter

- Des missions sans aucun chiffre ("Développement commercial", "Suivi clients")
- Ne pas préciser le type de vente (B2B, B2C, cycle long ou court)
- Oublier de mentionner le CRM utilisé
- Un CV trop générique qui ne dit rien de votre secteur d'expertise

## Template recommandé

Le template "Commercial" de DidCV met en avant vos chiffres clés dès le premier regard, avec une mise en page pensée pour les profils orientés résultats.
    `
  },
  {
    slug: 'cv-developpeur-erreurs',
    titre: 'CV développeur : les erreurs à éviter',
    description: 'Les erreurs les plus fréquentes dans les CV de développeurs et comment présenter votre stack technique efficacement.',
    date: '2025-07-18',
    categorie: 'CV',
    duree: '6 min',
    contenu: `
## Les recruteurs tech ne lisent pas un CV comme les autres

Un recruteur technique ou un lead dev passe en moyenne 30 secondes sur un CV développeur. Il cherche votre stack, votre niveau et vos projets concrets, pas un discours marketing.

## Les erreurs les plus courantes

### 1. Une liste de technologies sans hiérarchie
Lister 40 technologies sans préciser votre niveau réel décrédibilise votre profil. Distinguez ce que vous maîtrisez en production de ce que vous avez juste testé.

### 2. Pas de lien GitHub ou de portfolio
Pour un développeur, l'absence de lien vers du code réel est une occasion manquée. Même un profil GitHub modeste vaut mieux que rien.

### 3. Des missions non techniques
"A participé au développement de l'application" ne dit rien. Précisez : quel langage, quelle architecture, quel impact (performance, utilisateurs, revenus).

### 4. Ignorer les soft skills en équipe Agile
Les recruteurs tech valorisent aussi la collaboration : revue de code, pair programming, mentorat, participation aux rituels Scrum.

### 5. Un CV visuellement trop chargé
Les développeurs sont jugés aussi sur la simplicité de leur code. Un CV épuré et bien structuré est perçu positivement.

## Comment présenter sa stack technique

Organisez vos compétences par catégorie plutôt qu'en vrac :
- **Langages :** JavaScript, TypeScript, Python
- **Front-end :** React, Vue.js
- **Back-end :** Node.js, Express
- **Data / Infra :** PostgreSQL, Docker, AWS

## Ce qu'il faut absolument inclure

- Vos projets personnels ou open source
- Votre profil GitHub et/ou portfolio
- Le contexte technique de vos expériences (mono-repo, microservices, CI/CD)
- Votre niveau réel (junior, confirmé, senior) sans le surestimer

## Template recommandé

Le template "Tech" de DidCV structure automatiquement votre stack technique en catégories lisibles pour les recruteurs et les ATS.
    `
  },
  {
    slug: 'trouver-emploi-rapidement',
    titre: 'Comment trouver un emploi rapidement en 2025',
    description: 'Stratégies concrètes pour accélérer votre recherche d\'emploi : candidatures ciblées, réseau, plateformes et suivi.',
    date: '2025-07-20',
    categorie: 'Recherche d\'emploi',
    duree: '8 min',
    contenu: `
## Trouver un emploi vite : une question de méthode

La recherche d'emploi qui traîne est souvent une recherche mal organisée : candidatures dispersées, pas de suivi, pas de ciblage. Une méthode structurée réduit considérablement les délais.

## 1. Cibler avant de postuler massivement

Postuler à 200 offres au hasard est moins efficace que postuler à 30 offres parfaitement ciblées. Définissez précisément : le poste, le secteur, la zone géographique et le niveau de salaire visé.

## 2. Optimiser son CV pour chaque candidature

Un CV générique passe moins bien les filtres ATS. Adaptez le titre, l'accroche et les mots-clés à chaque offre. DidCV permet de générer un CV adapté à une offre en quelques secondes.

## 3. Activer tous les canaux en parallèle

- **Job boards** : France Travail, LinkedIn, Indeed, Welcome to the Jungle
- **Réseau** : contactez d'anciens collègues, camarades de promo, votre réseau LinkedIn
- **Candidatures spontanées** : ciblez les entreprises qui vous intéressent même sans offre publiée
- **Cabinets de recrutement et agences d'intérim** : utiles pour des postes non publiés

## 4. Soigner le réseau (le canal le plus rapide)

**Plus de 60% des postes** sont pourvus via le réseau avant même d'être publiés. Activez votre réseau LinkedIn, informez votre entourage de votre recherche et participez à des événements professionnels.

## 5. Suivre ses candidatures rigoureusement

Sans suivi, on perd du temps à relancer au mauvais moment ou à oublier de relancer du tout. Un tableau de suivi (comme celui de DidCV) permet de visualiser où en est chaque candidature et d'agir au bon moment.

## 6. Se préparer aux entretiens en avance

Ne pas attendre d'avoir un entretien programmé pour s'entraîner. Plus vous êtes préparé, plus vous transformez rapidement les entretiens en offres.

## Le rythme à tenir

Une recherche active efficace, c'est environ 5 à 10 candidatures ciblées par semaine, un suivi systématique, et du temps consacré chaque semaine au réseau.
    `
  },
  {
    slug: 'alternance-decrocher-contrat',
    titre: 'Alternance 2025 : comment décrocher son contrat',
    description: 'Guide pratique pour trouver une entreprise en alternance : CV, lettre, entretien et timing à respecter.',
    date: '2025-07-22',
    categorie: 'Alternance',
    duree: '7 min',
    contenu: `
## L'alternance : un recrutement à part

Décrocher un contrat d'alternance suit des règles différentes d'un recrutement classique. Le timing, la double contrainte école/entreprise et l'absence d'expérience changent la donne.

## Quand commencer ses recherches ?

Commencez idéalement **4 à 6 mois avant la rentrée**. Les entreprises reçoivent un pic de candidatures entre avril et juillet. Être parmi les premiers augmente vos chances.

## Le CV alternance qui fait la différence

### Mettez votre formation en avant
Indiquez clairement l'école, le diplôme préparé, le rythme d'alternance ("3 semaines entreprise / 1 semaine école") et la durée du contrat.

### Valorisez vos projets et stages
Sans grande expérience, vos projets académiques et stages précédents deviennent votre argument principal.

## La lettre de motivation en alternance

Expliquez précisément pourquoi cette entreprise, pourquoi ce poste, et comment votre formation s'y rattache. Montrez votre disponibilité et votre motivation pour un engagement de longue durée.

## Réussir l'entretien alternance

### Connaître son rythme d'alternance par cœur
Les recruteurs veulent savoir immédiatement si votre rythme est compatible avec leurs besoins.

### Montrer sa capacité d'apprentissage
En l'absence d'expérience professionnelle solide, la motivation et la capacité à apprendre vite sont décisives.

### Poser des questions sur l'accompagnement
Demandez qui sera votre tuteur, comment se passe l'intégration. Cela montre votre sérieux.

## Les canaux à activer

- Le CFA ou l'école (souvent en lien direct avec des entreprises partenaires)
- Les forums et salons alternance
- Les plateformes dédiées (La Bonne Alternance, HelloWork)
- Les candidatures spontanées auprès d'entreprises ciblées

## Ne pas se décourager

Le taux de réponse en alternance est souvent plus faible qu'en CDI classique. Multipliez les candidatures et restez actif jusqu'à la signature du contrat.
    `
  },
  {
    slug: 'lettre-motivation-stage',
    titre: 'Lettre de motivation pour un stage : guide complet',
    description: 'Comment rédiger une lettre de motivation efficace pour décrocher un stage, avec structure et exemples.',
    date: '2025-07-24',
    categorie: 'Lettre',
    duree: '6 min',
    contenu: `
## Pourquoi la lettre compte particulièrement pour un stage

Sans expérience professionnelle solide, la lettre de motivation devient un élément clé pour convaincre une entreprise de vous faire confiance sur une courte période.

## La structure d'une lettre de stage efficace

### Paragraphe 1 : pourquoi cette entreprise
Montrez que vous connaissez l'entreprise, son secteur et ses activités. Évitez les formules passe-partout.

### Paragraphe 2 : pourquoi ce stage correspond à votre formation
Expliquez le lien entre votre cursus, vos matières étudiées et le poste de stage visé.

### Paragraphe 3 : ce que vous pouvez apporter
Même sans expérience professionnelle, mettez en avant vos projets, compétences techniques et qualités personnelles pertinentes pour la mission.

### Conclusion
Précisez vos dates de disponibilité et proposez un entretien.

## Exemple d'accroche pour une lettre de stage

"Étudiante en 3ème année de licence Marketing à [École], je suis à la recherche d'un stage de 4 mois à partir de janvier 2026. Votre approche du marketing digital, notamment [élément précis], correspond exactement au type d'expérience que je recherche pour concrétiser mes compétences."

## Les erreurs fréquentes

- Une lettre trop générique envoyée à toutes les entreprises
- Ne pas préciser la durée et les dates de disponibilité du stage
- Oublier de mentionner le cadre académique (convention de stage, école)
- Recopier simplement le CV sans apporter d'éléments nouveaux

## Ce que les entreprises attendent d'un stagiaire

- Une réelle motivation pour le secteur et l'entreprise
- De la rigueur et de la ponctualité
- Une capacité d'apprentissage rapide
- Une bonne intégration dans l'équipe

## Conseil DidCV

Personnalisez chaque lettre en fonction de l'offre : une lettre générique se repère immédiatement par un recruteur habitué à en lire des dizaines chaque semaine.
    `
  },
  {
    slug: 'questions-pieges-entretien',
    titre: 'Les questions pièges en entretien et comment y répondre',
    description: 'Décryptage des questions difficiles en entretien d\'embauche et des réponses qui font la différence.',
    date: '2025-07-26',
    categorie: 'Entretien',
    duree: '8 min',
    contenu: `
## Pourquoi les recruteurs posent des questions pièges

Ces questions ne cherchent pas à vous déstabiliser gratuitement : elles évaluent votre honnêteté, votre capacité de recul et votre gestion du stress.

## "Quel est votre principal défaut ?"

**Le piège :** répondre par un faux défaut déguisé en qualité ("je suis trop perfectionniste").

**La bonne approche :** citez un vrai axe de progrès, avec les actions concrètes que vous mettez en place pour le travailler. Exemple : "J'ai tendance à vouloir tout vérifier moi-même ; j'apprends à déléguer davantage en donnant des points d'étape clairs à mon équipe."

## "Pourquoi avez-vous quitté votre précédent poste ?"

**Le piège :** critiquer ouvertement votre ancien employeur.

**La bonne approche :** restez factuel et positif. Parlez d'évolution de carrière, de recherche de nouveaux défis, sans dénigrer personne.

## "Où vous voyez-vous dans 5 ans ?"

**Le piège :** donner une réponse trop vague ou trop ambitieuse par rapport au poste proposé.

**La bonne approche :** montrez une trajectoire cohérente avec le poste et l'entreprise, tout en restant réaliste.

## "Parlez-moi d'un échec professionnel"

**Le piège :** dire que vous n'avez jamais échoué.

**La bonne approche :** utilisez la méthode STAR (Situation, Tâche, Action, Résultat) pour raconter un vrai échec et surtout ce que vous en avez appris.

## "Pourquoi devrions-nous vous embaucher plutôt qu'un autre candidat ?"

**Le piège :** répéter simplement votre CV.

**La bonne approche :** reliez précisément vos compétences aux besoins spécifiques du poste, avec un exemple concret de résultat obtenu.

## "Avez-vous des questions ?"

**Le piège :** répondre "non, tout est clair", cela peut être perçu comme un manque d'intérêt.

**La bonne approche :** préparez 2 à 3 questions sur l'équipe, les enjeux du poste ou les prochaines étapes du processus.

## La règle générale

Anticipez ces questions avant chaque entretien et préparez des réponses honnêtes, structurées et orientées vers ce que vous avez appris ou accompli.
    `
  },
  {
    slug: 'cv-btp-chantier',
    titre: 'CV BTP et chantier : guide complet 2025',
    description: 'Comment rédiger un CV efficace pour les métiers du BTP : habilitations, chantiers et compétences terrain.',
    date: '2025-07-28',
    categorie: 'CV',
    duree: '6 min',
    contenu: `
## Ce que recherchent les recruteurs du BTP

Dans le BTP, le CV doit avant tout rassurer sur la sécurité, les qualifications et l'expérience terrain concrète. Les recruteurs vont directement chercher vos habilitations et vos chantiers réalisés.

## Les éléments indispensables

### Les habilitations et certifications
Habilitations électriques, CACES, permis de conduire spécifiques, certifications gaz : mentionnez-les avec leur date de validité. C'est souvent le premier critère de sélection.

### Le type de chantiers
Précisez si vous avez travaillé sur des chantiers résidentiels, tertiaires, industriels ou publics. Le contexte compte autant que la compétence technique.

### Les outils et engins maîtrisés
Nacelle, grue, engins de chantier, logiciels de métré : soyez précis sur votre niveau de maîtrise réel.

## Structurer les missions

Décrivez vos missions de manière concrète et mesurable :
- "Installation et mise en service de tableaux électriques TGBT sur chantier tertiaire"
- "Coordination avec 4 corps de métier sur un chantier de 2000 m²"
- "Respect des délais sur 12 chantiers consécutifs sans dépassement de budget"

## La sécurité, un point non négociable

Le secteur du BTP valorise fortement la culture sécurité. Mentionnez vos formations sécurité (SST, habilitations spécifiques) et votre respect des normes en vigueur.

## Les compétences transversales à ne pas oublier

- Lecture de plans et schémas techniques
- Travail en équipe et coordination inter-métiers
- Autonomie et gestion des imprévus sur chantier
- Rigueur et respect des délais

## Template recommandé

Le template "BTP" de DidCV met en avant vos habilitations et vos chantiers dès le premier coup d'œil, dans un format clair et professionnel apprécié des recruteurs du secteur.
    `
  },
  {
    slug: 'reconversion-par-ou-commencer',
    titre: 'Reconversion professionnelle : par où commencer',
    description: 'Les premières étapes concrètes pour réussir sa reconversion professionnelle sans se précipiter.',
    date: '2025-07-30',
    categorie: 'Reconversion',
    duree: '7 min',
    contenu: `
## La reconversion : un projet qui se construit étape par étape

Se reconvertir ne s'improvise pas. Une reconversion réussie repose sur une méthode claire plutôt que sur un saut dans l'inconnu.

## Étape 1 : Faire le bilan de son parcours

Avant de choisir une nouvelle voie, listez vos compétences acquises, vos réussites et ce qui vous a le plus motivé dans vos expériences passées. Un bilan de compétences peut vous y aider.

## Étape 2 : Identifier des pistes réalistes

Croisez trois critères : vos compétences transférables, vos centres d'intérêt réels et les débouchés du marché de l'emploi dans le secteur visé. Une reconversion viable se situe à l'intersection des trois.

## Étape 3 : Se renseigner sur le terrain

Avant de vous engager dans une formation longue, échangez avec des professionnels du métier visé (réseau, immersions, stages courts) pour confirmer que la réalité du métier correspond à votre projection.

## Étape 4 : Identifier la formation nécessaire

Toutes les reconversions ne nécessitent pas une formation longue et coûteuse. Certifications courtes, bootcamps, formations en ligne ou VAE (Validation des Acquis de l'Expérience) peuvent suffire selon le métier visé.

## Étape 5 : Financer son projet

Renseignez-vous sur le Compte Personnel de Formation (CPF), le Projet de Transition Professionnelle (PTP), ou les dispositifs France Travail dédiés à la reconversion.

## Étape 6 : Adapter son CV et sa candidature

Structurez votre CV autour des compétences transférables plutôt que de la chronologie stricte, et expliquez votre reconversion de manière positive dans votre accroche.

## Étape 7 : Se lancer progressivement

Il n'est pas nécessaire de tout quitter du jour au lendemain. Un projet à temps partiel, un stage ou une mission courte peuvent constituer une transition en douceur vers votre nouveau métier.

## L'essentiel à retenir

Une reconversion réussie est une reconversion préparée : bilan, exploration terrain, financement identifié et candidature adaptée sont les quatre piliers du succès.
    `
  },
]

export const CATEGORIES = [...new Set(ARTICLES.map(a => a.categorie))]
